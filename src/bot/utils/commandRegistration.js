...
const USE_GUILD_COMMANDS = true; // <--- Set to true for per-guild, false for global
	...
	for (const file of files) {
		...
		if (stat.isDirectory()) {
			if (!file.startsWith("_")) {
				...
				console.log(`ℹ️ Skipping disabled directory: ${path.join(relPath, file)}`);
			}
		} else if (
			stat.isFile() &&
			file.endsWith(".js") &&
		...
		for (const { filePath, category } of commandFiles) {
			try {
				...
				if (commandModule.data) {
					...
					console.log(`✅ Registered command: /${commandData.name}${category ? ` (category: ${category})` : ""}`);
					...
					console.log(`⚠️ Using default configuration for command: /${defaultName}${category ? ` (category: ${category})` : ""}`);
				}
			} catch (error) {
...
export async function registerCommands() {
	...
	try {
		...
		console.log(`✅ Successfully registered ${data.length} slash commands ${targetDescription}:`);
		for (const cmd of manifest) {
			console.log(`   - /${cmd.name}${cmd.category ? ` (category: ${cmd.category})` : ""}`);
		}
		...
		if (error.code === 429) {
			...
			console.error(`   Retry after: ${error.retry_after || 'unknown'} seconds`);
			console.error(`   Rate limit bucket: ${error.bucket || 'unknown'}`);
			console.error(`   Global rate limit: ${error.global || false}`);

			if (error.retry_after) {
				console.error(`   ⏰ Wait ${error.retry_after} seconds before retrying`);
			}
		...
		console.error(`   Token length: ${process.env.DISCORD_TOKEN?.length || 0} characters`);
		...
		if (error.message?.includes('timeout')) {
			...
			console.error("   - Consider registering commands per guild instead of globally");
		} else if (error.code === 401) {
