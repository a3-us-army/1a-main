...
export default function modmailHandler(client) {
    ...
    client.on("messageCreate", async (message) => {
        ...
        const modmailChannel = await findOrCreateModmailChannel(message.author, client);

        // If this is the first message in the channel, send a welcome message to the user
        if (modmailChannel.lastMessageId === null) {
            await message.author.send({
                content: "👋 Thank you for contacting support! A staff member will respond to you here as soon as possible. Please describe your issue in detail.",
            }).catch(() => {});
        }

        // Post the user's message as embed with buttons, and store mapping
        const modmailMsg = await postUserMessageToChannel(message, modmailChannel, true);
    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if (customId === "close_thread") {
            ...
            await interaction.reply({ content: "Thread closed and transcript uploaded.", ephemeral: true });
