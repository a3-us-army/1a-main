...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(opt =>
    opt
      .setName('users')
      .setDescription('User mentions or IDs, separated by spaces or commas')
      .setRequired(false)
  )
...
export async function execute(interaction) {
    ...
    if (userIds) {
      for (const id of userIds) {
        try {
          ...
          if (user && !user.bot && !users.find(u => u.id === user.id)) {
            users.push(user);
          }
        } catch {
          // Ignore fetch errors
  ...
  if (role) {
    ...
    for (const member of roleMembers.values()) {
      if (!users.find(u => u.id === member.user.id) && !member.user.bot) {
  ...
  for (const user of users) {
    try {
      ...
      failed++;
      failedUsers.push(
        user.tag ? `${user.tag} (<@${user.id}>)` : `<@${user.id}>`
      );
    }
  }
