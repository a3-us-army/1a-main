...
router.get('/', ensureAuth, async (req, res) => {
  const forms = db
    .prepare('SELECT * FROM custom_forms ORDER BY created_at DESC')
    .all();
  let isAdmin = false;
  if (req.user) {
    isAdmin = await isUserAdmin(req.user.id);
  }
  res.render('forms/forms_list', {
    user: req.user,
    forms,
    active: 'forms',
...
router.get('/admin/new', ensureAuth, ensureAdmin, async (req, res) => {
  ...
  res.render('admin/forms_admin_new', {
    user: req.user,
...
router.get('/:id', ensureAuth, (req, res) => {
  const form = db
    .prepare('SELECT * FROM custom_forms WHERE id = ?')
    .get(req.params.id);
  if (!form) return res.redirect('/forms?error=Form not found');
  form.fields = JSON.parse(form.fields);
  res.render('forms/forms_fill', {
    user: req.user,
    form,
    active: 'forms',
    alert: req.query.alert,
...
router.post('/:id', ensureAuth, async (req, res) => {
  ...
  for (const field of fields) {
    ...
    if (field.required && !submission[field.name]) {
      return res.redirect(
        `/forms/${form.id}?error=Missing required field: ${field.label}`
      );
    }
  }
  // Post to Discord
  try {
    ...
    await fetch(
      process.env.BOT_API_URL.replace(/\/api\/post-event$/, '/api/post-form'),
      {
        method: 'POST',
        headers: {
...
router.post('/admin/edit/:id', ensureAuth, ensureAdmin, (req, res) => {
  ...
  if (!title || !field_names || !field_labels) {
    return res.redirect(
      `/forms/admin/edit/${req.params.id}?error=Missing required fields`
    );
  }
  // Build fields array
  const fields = [];
